...
(TransactionID Serial PRIMARY KEY, Customer_name VARCHAR,state VARCHAR,
email VARCHAR, gender VARCHAR,
FOREIGN KEY (TransactionID) REFERENCES Customer(TransactionID));

copy Customer(TransactionID, Customer_name, state, email,gender)
from 'D:\DA Technical Courses\SQL Repository\Dataset\Customer Demographics Datset.csv'
DELIMITER ','
CSV HEADER;
